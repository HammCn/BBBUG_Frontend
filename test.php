...
<body>
    <div id="app" v-cloak>
        <div id="main" v-loading.fullscreen.lock="loading">
            ...
            <el-card class="box-card singe chat_room" shadow="never" v-if="room.roomInfo">
                ...
                <div slot="header" class="clearfix">
                    <span>
                        <el-tag size="mini" type="info" style="margin: 0px 5px;color:#333;font-weight:bolder;font-size:14px;">
                            ID:{{room.room_id}}
                        </el-tag>
                        <i class="iconfont room_icon icon-xiaoxi2" v-if="room.roomInfo.room_type==0"></i>
                        <i class="iconfont room_icon icon-changyongtubiao-mianxing-61" v-if="room.roomInfo.room_type==1"></i>
                        <i class="iconfont room_icon icon-shezhi3" v-if="room.roomInfo.room_type==2"></i>
                        <i class="iconfont room_icon icon-book" v-if="room.roomInfo.room_type==3"></i>
                    ...
                    <span style="float: right; padding: 3px 0" class="rightMenu">
                        <el-button-group style="text-align: right;">
                            ...
                            <el-button size="mini" @click="doShowRoomList" style="color:orangered"
                ...
                <div class="chat_room_box">
                    ...
                    <el-alert
                        :title="systemTips.msg" v-if="systemTips.msg"
                        :type="ystemTips.type"
                        effect="dark">
                    </el-alert>
                    <div class="chat_room_toolbar">
                        ...
                        <div v-if="((room.roomInfo.room_type==1||room.roomInfo.room_type==2) && chat_room.song) || (room.roomInfo.room_type==3 && chat_room.voice)"
                            ...
                            <div class="player_title">
                                ...
                                <i class=" iconfont" @click="doSwitchMusic" @mouseover="doShowVoiceBar"
                                    title="音乐开关" style='font-weight:normal' :style="volume==0?'color:#999':'color:#fff'" :class="volume==0?'icon-changyongtubiao-xianxingdaochu-zhuanqu-40':'icon-changyongtubiao-xianxingdaochu-zhuanqu-39'"></i>
                            </div>
                ...
                <!-- 房间列表开始 -->
                <el-drawer title="房间面板" direction="ltr" ref="online_box" :visible.sync="room.showDialog"
                        :modal-append-to-body='false' :with-header="false" size="320px" box-card singe custom-class="room_list">
                    <div class="room_header">
                        <span class="room_title">热门房间推荐</span>
                        <span style="float: right; padding: 3px 0">
                            <el-button-group>
                                <el-button size="mini" type="success" v-if="!userInfo.myRoom" @click="doCreateRoom"
                    ...
                    <div class="room_body" >
                        <div class="room_item" title="进入房间" v-for="(row,index) in room.list" @click="doJoinRoomById(row.room_id)" >
                            <!-- <i class="room_icon el-icon-medal"></i>
                            <i class="room_icon el-icon-map-location"></i> -->
                            <div class="room_name">
                                <i class="room_icon el-icon-lock" v-if="row.room_public==1" title="密码房间"></i>
                                <i class="room_icon el-icon-chat-line-round" v-if="row.room_type == 0"
                                    title="普通文字聊天房"></i>
                                <i class="room_icon el-icon-service" v-if="row.room_type == 1"
                                    title="一起听歌音乐房"></i>
                                <i class="room_icon el-icon-coin" v-if="row.room_type == 2"
                                    title="听歌猜歌游戏房"></i>
                                <i class="room_icon el-icon-notebook-2" v-if="row.room_type == 3"
                                    title="听书故事相声房"></i>
                                    {{ row.room_name }}
                                    <font color=orangered style="font-weight: bolder;" v-if="row.room_online>0">
                                    ({{row.room_online}})</font>
                            </div>
                            <div class="room_id">ID:{{row.room_id}}</div>
                                <img class="room_img" :src="row.user_head" onerror="this.src='images/nohead.jpg'"/>
                            <div class="room_master">
                                <div class="room_info">
                                    <div class="room_user">{{urldecode(row.user_name)}}<el-tag size="mini"
                                        type="warning" class="isAdmin" title="管理员" v-if="row.user_admin">管
                                        </el-tag>
                                    </div>
                ...
                <el-form :model="login.form" ref="login_form" label-width="60px">
                    ...
                    { required: true, message: '不填写密码如何登录???', trigger: 'blur' }
                        ...
                        <el-input v-model="login.form.user_password" type="password" placeholder="请输入你的登录密码或验证码"
                            ...
                            <el-button slot="append" icon="el-icon-message" @click="doSendRandCode"
        ...
        <audio :src="audioUrl" id="audio" ref="audio" autoplay="autoplay" controlss @timeupdate="audioTimeUpdate"></audio>
        ...
        var BBBUG = new Vue({
            ...
            methods: {
                    ...
                    that.$refs.audio.addEventListener("canplay", function () {
                        if(!that.isAudioCurrentTimeChanged){
                            ...
                            switch(that.room.roomInfo.room_type){
                                case 1:
                                case 2:
                                    that.$refs.audio.currentTime = (nowTimeStamps - that.chat_room.song.since);
                                break;
                                case 3:
                                    that.$refs.audio.currentTime = (nowTimeStamps - that.chat_room.voice.since);
                                break;
                            }
                        }
                ...
                addSystemTips(msg,type='info') {
                    ...
                    that.systemTips = msg;
                    setTimeout(function(){
                        that.systemTips.msg = false;
                        that.systemTips.type = type;
                    },3000);
                },
                autoScroll() {
                    let that = this;
                        ...
                        switch (obj.type) {
                            ...
                            case 'addSong':
                                if (obj.at) {
                                    ...
                                    that.addSystemTips(that.urldecode(obj.user.user_name) + " 点了一首 《" + obj.song.name + "》(" + obj.song.singer + ")", '#409EFF', '#eee');
                                }
                                that.saveMessageHistory();
                                break;
                            case 'chat_bg':
